...
// Inline SVG area+line chart of end-of-day balance over time.
function balanceChart(acct) {
  ...
  const dots = h.map((d, i) => `<circle cx="${xAt(i).toFixed(1)}" cy="${yAt(d.balance).toFixed(1)}" r="3" fill="${GREEN}"/>`).join('');
  const xlabels = h.map((d, i) => `<text x="${xAt(i).toFixed(1)}" y="${H - 8}" text-anchor="middle" class="ax">${dayTick(d.date)}</text>`).join('');
